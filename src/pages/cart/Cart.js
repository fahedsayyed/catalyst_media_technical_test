      ...
      <div className="masthead">
        ...
        />
        {
          cartItems.map((cartItem)=>{
            return(
              <>
              <div className="home-w-cart-update-child" />
              <div className="shopping-cart">Shopping Cart</div>
              <div className="product-listing6">
                <div className="image12">
                  <div className="image1">
                    <div className="rectangle-parent3">
                      <div className="frame-child3" />
                      <img
                        className="new-project-30-16"
                        alt=""
                        src={cartItem.image}
                      />
                    </div>
                ...
                <div className="content-parent">
                  <div className="content6">
                    <div className="autumn-flower-top-parent">
                      <div className="autumn-flower-top">{cartItem.name}</div>
                      <div className="black-parent">
                        <b className="black">{colors.find((c) => c.id === cartItem.colorId)
                          ? colors.find((c) => c.id === cartItem.colorId).name
                          : "BLACK"}</b>
                        <div className="black">{materials.find((c) => c.id === cartItem.materialId)
                          ? materials.find((c) => c.id === cartItem.materialId).name
                          : "COTTON"}</div>
                      </div>
                    </div>
                    <div className="inr-219900-wrapper">
                      <div className="inr-219900">INR {cartItem.price}</div>
                    </div>
                  </div>
                  <div className="remove-parent" onClick={()=>handleDelete(cartItem.id)}>
                    <b className="filter">Remove</b>
                    <b className="filter">X</b>
                  </div>
                </div>
              </div>
              </>
